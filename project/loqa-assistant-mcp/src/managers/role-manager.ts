...
export class LoqaRoleManager {
    ...
    try {
      ...
      for (const roleInfo of this.roleSystemConfig.available_roles) {
        ...
        try {
          ...
          this.roleConfigs.set(roleInfo.role_id, {
            ...
            issue_templates_preferred:
              roleConfig.issue_templates_preferred || [],
          });
        } catch (error) {
          console.warn(
            `Failed to load role config for ${roleInfo.role_id}:`,
            error
          );
        }
      }
      ...
      if (this.roleConfigs.size === 0) {
        this.roleConfigs.set("general", {
          ...
          issue_templates_preferred: ["general-task-template"],
        });
      }
    }
  }
  ...
  /**
   * Detect appropriate role based on issue context
   */
  async detectRole(context: {
    ...
    for (const [roleId, roleConfig] of this.roleConfigs) {
      ...
      if (context.repositoryType) {
        const architectRepos = [
          ...getRepositoriesByType("protocol"),
          ...getRepositoriesByType("core"),
        ];
        const developerRepos = [
          ...getRepositoriesByType("service"),
          ...getRepositoriesByType("client"),
        ];
        const coreRepos = getRepositoriesByType("core");

        if (
          roleId === "architect" &&
          architectRepos.includes(context.repositoryType)
        ) {
          scores[roleId].score += 2;
        }
        if (
          roleId === "developer" &&
          developerRepos.includes(context.repositoryType)
        ) {
          scores[roleId].score += 2;
        }
      ...
      for (const filePath of context.filePaths || []) {
        ...
        if (
          roleId === "developer" &&
          (path.includes(".go") || path.includes(".ts") || path.includes(".js"))
        ) {
          scores[roleId].score += 2;
        }
        if (
          roleId === "architect" &&
          (path.includes(".proto") || path.includes("api"))
        ) {
          scores[roleId].score += 3;
        }
    ...
    const confidence =
      totalWords > 0 ? Math.min(topRole.score / totalWords, 1.0) : 0;

    // Apply confidence threshold
    const threshold =
      this.roleSystemConfig?.role_detection?.confidence_threshold || 0.6;
    const detectedRole =
  ...
  async getTemplatesForRole(roleId: string): Promise<string[]> {
    ...
    return roleConfig?.issue_templates_preferred || ["general-task-template"];
  }
